...
const QuantitySelector = ({
  ...
  console.log(
    "DEBUG - Current availability from useGenericCart:",
    availabilityStatus
  );
  console.log("DEBUG - Current cart quantity:", cartQuantity);
  ...
  const handleQuantityChangeLocal = useCallback(
    (newQuantity) => {
      ...
      console.log("🔥 Max available:", availabilityStatus.maxAvailable);

      // Validate quantity bounds
      if (newQuantity < 0) {
        console.log("🔥 Invalid quantity: cannot be negative");
        onError("Quantity cannot be negative");
        return;
      }

      if (newQuantity > availabilityStatus.maxAvailable) {
        console.log("🔥 Invalid quantity: exceeds max available");
  ...
  // ✅ NEW: Effect to notify parent when cart quantity actually changes
  useEffect(() => {
    onQuantityChange(cartQuantity);
